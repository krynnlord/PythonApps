...
def add_entry(vault: dict):
    ...
    site = input("\33[93mFirst Name: \33[0m")
    ...
    vault[site] = {'first_name': site, 'last_name' : last_name, 'title': title, 'phone_number' : phone_number, 'email' : email}
...
def view_entry_list(vault: dict, fernet: Fernet, filtered_sites=None):
    ...
    sites.sort(key=lambda site: vault[site]['last_name'])
        
    if not sites:
        return

    for i, site in enumerate(sites, start=1):
        print("\33[34m"+f"{i}. "+"\33[0m"+ f"{vault[site]['last_name']}" + ", " + f"{vault[site]['first_name']}" + "\33[0m")
    ...
    try: 
        ...
        print("\33[34mName:  \33[0m"+ vault[selected_site]['first_name'] + " " + vault[selected_site]['last_name'])
        print("\33[34mTitle: \33[0m"+ vault[selected_site]['title'])
        print("\33[34mPhone: \33[0m"+ vault[selected_site]['phone_number'])
        print("\33[34mEmail: \33[0m"+ vault[selected_site]['email'])
...
def search_vault(vault: dict, fernet: Fernet):
    ...
    matches = [site for site in vault if term in site.lower() or term in vault[site]['first_name'].lower() or term in vault[site]['first_name'].lower() or term in vault[site]['title'].lower() or term in vault[site]['phone_number'].lower() or term in vault[site]['email'].lower()]
    clear_screen()
